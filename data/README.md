# 合成数据集
本文件夹用于存放数据集（包括 公共数据集、自建数据集、数据观察）。下文主要围绕自建的合成数据集（因为没有非常切合本项目任务的数据集），论述数据的处理方法。

构建合成数据有两个要点：**音色**和**音符**。

首先关注**音色**。为了容易出效果，要用音色差别大的乐器的音频的混合。首先尝试了用[CQT](../model/CQT.py)度量音色，得到了频谱含义下音色之间的距离和聚类，详见[音色度量](#timbre)。因为要和midi打交道，本文的最后附上了[midi协议下的音色、音高编码表](#midi)。

然后关注**音符**。为了以后的数据可以“取之不尽”，决定随机生成midi，再合成为wav。生成、合成部分见[array→midi](#midiarray)、[midi→wav](#synth)；随机算法见[随机MIDI](#rand)。[数据尺寸](#size)明确了数据集元素的时间属性。
...
采用numpy数组作为可处理格式，含义是钢琴卷帘、时频形式。数据的ground truth是nparray而不是基于事件的midi，midi作为和其他格式的中转，比如和mir_eval库交互、和人类交互，但都发生在模型训练之后。因此生成数据集的顺序是：生成nparray→生成midi→生成wav→预计算CQT。
...
- midi -> nparray: 只关注了节拍和音符事件，丢失了强度信息
- nparray -> midi: bpm固定为120，tick固定为480，控制器只有乐器种类

## 数据尺寸{#size}
时长的考量：

CQT降采样结构的性质决定：8个八度导致时间间隔必须是128的整数倍。因此在22050点采样下，则最小时间单位为 $\frac{1000}{22050/128}=5.8ms$，太小了，不适合浏览器运行。考虑bpm120的音乐，32分音符是62.5ms，64分音符是31.25ms，要保证64分音符至少有两个采样点，应使用256点或者384点的间隔。
...
一开始追求转录速度和内存开销，使用了384点。然而后面发现用256点的转录结果更准确，所以从384换成了256。每个样本时长900帧，约10.45s。

VAE和最终的模型是按照变长设计的，但是要batch训练，要求数据一样长。torch提供了一些函数，用于不同长度的RNN训练，可参考[文章](https://blog.csdn.net/anshiquanshu/article/details/112868740)。懒得弄，索性都取900帧。
...
该notebook还实验了“同一个乐器、不同音符”的频谱。结论是：哪怕是合成的，频谱结构依旧和音高相关。
...
### 人工设计
且不管VAE生成结果如何，作为数据集还是不够合格，因为无法保证每一个音符都出现，不够稳定。在这个要求下，基于概率得到的数据集都不行，除非数据量很大。

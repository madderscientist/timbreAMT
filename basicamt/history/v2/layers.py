...
class CBR(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size = 3, stride = 1, padding = "same", dilation = 1):
        super().__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, bias=False, dilation = dilation)
        self.bn = nn.BatchNorm2d(out_channels)
...
class ChordConv(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = sum(conv(x) for conv in self.convs)
        x = self.bn(x)
        return self.act(x)


class ChordConv2(nn.Module):

...
class Notes:
    ...
    def fetch(self, last_note = None, near_future_len = 0):
        ...
        offset_dist = truncnorm(
            ...
            (self.len_range[1] + 0.5) / self.len_sigma, # 0.5因为round是四舍五入，需要补偿边沿
            loc=0,
            scale=self.len_sigma
        )
    ...
    def generate(self, frames = 660, polyphonic = 0.16, near = 0.7):
        """
        生成一个音符序列
        frames: 音符序列的长度
